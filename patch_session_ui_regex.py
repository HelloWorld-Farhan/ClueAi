...
pattern = r'<div className="space-y-1">\s*\{sessions\.length === 0 \? \([\s\S]*?<Trash2 size=\{14\} /> Delete\s*</button>\s*</div>\s*</>\s*\)}\s*</div>\s*\)\)}\s*</div>'
...
if match:
    ...
    print("Failed to find map block boundaries via regex.")
