                    ...
                    {expandedSessionId === session.id && (
                      <div className="border-t border-brand-border bg-black/40 p-4 space-y-4">
                        {(!session.history || session.history.length === 0) ? (
                          ...
                          return (
                            <div key={idx} className="bg-white/5 border border-white/5 rounded-lg overflow-hidden cursor-default" onClick={(e) => e.stopPropagation()}>
                              
                              {/* Transcript Header */}
                              <div 
                                className="px-4 py-3 bg-white/5 hover:bg-white/10 cursor-pointer flex justify-between items-center transition-colors"
                                onClick={(e) => { e.stopPropagation(); toggleHistoryItem(session.id, idx, 'transcript'); }}
                              >
                              ...
                              <div 
                                ...
                                onClick={(e) => { e.stopPropagation(); toggleHistoryItem(session.id, idx, 'answer'); }}
                              >
...
if start_idx != -1 and end_idx != -1:
    ...
    print("Failed to find map block boundaries.")
