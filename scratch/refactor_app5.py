...
# Insert accordions BEFORE the closing tags of the settings modal
# The structure is:
#                   </div> (ends Gemini accordion content)
#                 </div> (ends Gemini Accordion wrapper)
#               </div> (ends Settings accordion list)
#             </section>
match = re.search(r'(\s*</div>\s*</div>\s*</section>)', code)
if match:
    # Actually wait! Gemini Accordion ends with:
    #                        ))}
    #                      </div>
    #                    )}
    #                  </div>
    # Wait, the end of the gemini section has </div> then </div> then </section>.
    # I'll just find `{/* Interview Context */}` and go back.
    idx = code.find("{/* Interview Context */}")
    if idx != -1:
        # Find the </section> before Interview Context
        section_idx = code.rfind("</section>", 0, idx)
        if section_idx != -1:
            # We want to insert the accordions INSIDE the section. 
            # The section closes the api key list, so the inner `</div>` is where we insert.
            # Let's insert right before `</div>\n              </section>`
            insert_idx = code.rfind("</div>", 0, section_idx)
            if insert_idx != -1:
                code = code[:insert_idx] + accordions + code[insert_idx:]
